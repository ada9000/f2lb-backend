...
const poolMock = {
  ...
  description:
    "Pixel believes in a decentralized future achieved with sustainable solutions. Will always be a single pool operator donating profits to environmental projects.",
  epochsGranted: 2,
  epochs: [400, 401],
  numEpochs: 2,
  queuePos: 10,
...
const poolsMock = JSON.parse(
  fs.readFileSync("__tests__/queueMock.json", "utf8")
);
...
describe("ada conversions", () => {
  ...
  describe("update Allowed epochs", () => {
    it("over 1k ada gives 1 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(1000.1) });
    ...
    it("over 3k ada gives 2 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(3000.1) });
      const updatedPool = await updateAllowedEpochs(poolMock, 300);
    ...
    it("over 10k ada gives 3 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(10000.1) });
      const updatedPool = await updateAllowedEpochs(poolMock, 300);
    ...
    it("over 40k ada gives 4 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(40000.1) });
      const updatedPool = await updateAllowedEpochs(poolMock, 300);
    ...
    it("exactly 1k ada gives 0 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(1000) });
      const updatedPool = await updateAllowedEpochs(poolMock, 300);
    ...
    it("under 1k ada gives 0 assigned epochs", async () => {
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(100) });
      const updatedPool = await updateAllowedEpochs(poolMock, 300);
      expect(updatedPool.epochsGranted).toBe(0);
    });
    ...
    it("in next 7, reduced stake from over 4k to 10k same blocks allowed", async () => {
      ...
      koios.accountInfo = jest
        .fn()
        .mockReturnValue({ total_balance: adaToLace(10000) });
      const updatedPool = await updateAllowedEpochs(updatedPoolMock, 300);
      expect(updatedPool.epochsGranted).toBe(2);
    });
  ...
  describe("update status", () => {
    ...
    it("pool wallet doesn't match target", async () => {
      let updatedPoolMock = JSON.parse(JSON.stringify(poolMock));
      updatedPoolMock.wallet.delegation =
